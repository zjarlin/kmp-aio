...
// 命名管道名称
const wchar_t* IPCClient::PIPE_NAME = L"\\\\.\\pipe\\KCloud";
...
// Shell 扩展实现
KCloudContextMenu::KCloudContextMenu() : m_cRef(1) {
}

KCloudContextMenu::~KCloudContextMenu() {
}

IFACEMETHODIMP KCloudContextMenu::QueryInterface(REFIID riid, void** ppv) {
    static const QITAB qit[] = {
        QITABENT(KCloudContextMenu, IShellExtInit),
        QITABENT(KCloudContextMenu, IContextMenu),
        { 0 }
    };
    return QISearch(this, qit, riid, ppv);
}

IFACEMETHODIMP_(ULONG) KCloudContextMenu::AddRef() {
    return InterlockedIncrement(&m_cRef);
}

IFACEMETHODIMP_(ULONG) KCloudContextMenu::Release() {
...
IFACEMETHODIMP KCloudContextMenu::Initialize(LPCITEMIDLIST pidlFolder, 
                                               LPDATAOBJECT pdtobj, 
                                               HKEY hkeyProgID) {
    if (pdtobj == NULL) {
        return E_INVALIDARG;
...
IFACEMETHODIMP KCloudContextMenu::QueryContextMenu(HMENU hmenu, 
                                                    UINT indexMenu, 
                                                    UINT idCmdFirst, 
                                                    UINT idCmdLast, 
                                                    UINT uFlags) {
    ...
    ExpandEnvironmentStringsW(L"%USERPROFILE%\\KCloud", syncRoot, MAX_PATH);
    ...
    InsertMenu(hSubMenu, 0, MF_BYPOSITION | MF_STRING, idCmdFirst + ID_SYNC_NOW, L"KCloud - 立即同步");
    InsertMenu(hSubMenu, 1, MF_BYPOSITION | MF_STRING, idCmdFirst + ID_SHOW_IN_APP, L"KCloud - 在应用中显示");
    ...
    if (hasConflict) {
        InsertMenu(hSubMenu, 3, MF_BYPOSITION | MF_STRING, idCmdFirst + ID_RESOLVE_CONFLICT, L"KCloud - 解决冲突");
    }
    
    InsertMenu(hSubMenu, 4, MF_BYPOSITION | MF_STRING, idCmdFirst + ID_GET_SHARE_LINK, L"KCloud - 获取共享链接");
    ...
    mii.dwTypeData = const_cast<LPWSTR>(L"KCloud");
...
IFACEMETHODIMP KCloudContextMenu::InvokeCommand(LPCMINVOKECOMMANDINFO pici) {
...
IFACEMETHODIMP KCloudContextMenu::GetCommandString(UINT_PTR idCmd, 
                                                    UINT uType, 
                                                    UINT* pReserved, 
                                                    CHAR* pszName, 
                                                    UINT cchMax) {
...
std::string KCloudContextMenu::GetFileStatus(const std::wstring& path) {
    // 转换路径为 UTF-8
...
void KCloudContextMenu::SyncFile(const std::wstring& path) {
    int size = WideCharToMultiByte(CP_UTF8, 0, path.c_str(), -1, NULL, 0, NULL, NULL);
    std::string utf8Path(size - 1, 0);
    WideCharToMultiByte(CP_UTF8, 0, path.c_str(), -1, &utf8Path[0], size, NULL, NULL);
...
void KCloudContextMenu::ShowInApp(const std::wstring& path) {
    int size = WideCharToMultiByte(CP_UTF8, 0, path.c_str(), -1, NULL, 0, NULL, NULL);
    std::string utf8Path(size - 1, 0);
    WideCharToMultiByte(CP_UTF8, 0, path.c_str(), -1, &utf8Path[0], size, NULL, NULL);
...
void KCloudContextMenu::ResolveConflict(const std::wstring& path) {
    int size = WideCharToMultiByte(CP_UTF8, 0, path.c_str(), -1, NULL, 0, NULL, NULL);
    std::string utf8Path(size - 1, 0);
    WideCharToMultiByte(CP_UTF8, 0, path.c_str(), -1, &utf8Path[0], size, NULL, NULL);
...
public:
    ...
    IFACEMETHODIMP CreateInstance(IUnknown* pUnkOuter, REFIID riid, void** ppv) {
        ...
        KCloudContextMenu* pExt = new (std::nothrow) KCloudContextMenu();
...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, void** ppv) {
    if (IsEqualCLSID(rclsid, CLSID_KCloudContextMenu)) {
...
STDAPI DllRegisterServer() {
    HKEY hKey;
    wchar_t clsidStr[39];
    StringFromGUID2(CLSID_KCloudContextMenu, clsidStr, 39);
    
    // 注册 CLSID
    std::wstring clsidKey = L"CLSID\\" + std::wstring(clsidStr);
    if (RegCreateKeyExW(HKEY_CLASSES_ROOT, clsidKey.c_str(), 0, NULL, 0, 
                        KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS) {
        RegSetValueExW(hKey, NULL, 0, REG_SZ, (BYTE*)L"KCloud Context Menu", 
                       sizeof(L"KCloud Context Menu"));
    ...
    // 注册到右键菜单
    std::wstring menuKey = L"*\\shellex\\ContextMenuHandlers\\KCloud";
    ...
    // 注册目录右键菜单
    std::wstring dirKey = L"Directory\\shellex\\ContextMenuHandlers\\KCloud";
...
STDAPI DllUnregisterServer() {
    ...
    RegDeleteTreeW(HKEY_CLASSES_ROOT, L"*\\shellex\\ContextMenuHandlers\\KCloud");
    RegDeleteTreeW(HKEY_CLASSES_ROOT, L"Directory\\shellex\\ContextMenuHandlers\\KCloud");
