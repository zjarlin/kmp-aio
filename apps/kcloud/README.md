...
- 主壳现在通过 `site.addzero.buildlogic.kmp.cmp-kcloud-aio` 自动聚合 `apps/kcloud/plugins/**` 下的插件模块
...
新增插件只要放进 `apps/kcloud/plugins/**` 并满足命名约定，`composeApp`、`server`、`shared` 会在构建期自动聚合，不再需要手改壳层依赖列表。
