package com.kcloud.features.servermanagement.server.security
...
private val logger: Logger = Logger.getLogger("com.kcloud.features.servermanagement.server.security.EncryptionManager")
...
data class EncryptionHeader(
    ...
    companion object {
        val MAGIC = byteArrayOf(0x4B, 0x43, 0x45, 0x4E) // "KCEN"
        ...
        fun readFrom(input: InputStream): EncryptionHeader? {
            ...
            if (dataIn.read(magic) != 4 || !magic.contentEquals(MAGIC)) {
                logger.severe("Invalid magic number, not a KCloud encrypted file")
                return null
            }
...
class EncryptedStorageClient(
    ...
    override suspend fun uploadObject(
        ...
        // 创建临时加密文件
        val tempEncryptedFile = File.createTempFile("kcloud_enc_", ".tmp")
