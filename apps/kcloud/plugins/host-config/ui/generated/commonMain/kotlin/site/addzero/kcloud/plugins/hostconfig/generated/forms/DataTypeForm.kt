...
@Composable
fun DataTypeForm(
    state: MutableState<DataTypeIso>,
    visible: Boolean,
    title: String,
    onClose: () -> Unit,
    onSubmit: () -> Unit,
    confirmEnabled: Boolean = true,
    dslConfig: DataTypeFormDsl.() -> Unit = {}
) {
    AddDrawer(
        visible = visible,
        title = title,
        ...
        DataTypeFormOriginal(state, dslConfig)
    }
}

@Composable
fun DataTypeFormOriginal(
    state: MutableState<DataTypeIso>,
    dslConfig: DataTypeFormDsl.() -> Unit = {}
) {
    val renderMap = remember { mutableMapOf<String, @Composable () -> Unit>() }
    val dsl = DataTypeFormDsl(state, renderMap).apply(dslConfig)
    val defaultRenderMap = linkedMapOf<String, @Composable () -> Unit>(
        DataTypeFormProps.createdAt to {
            AddTextField(
        ...
        DataTypeFormProps.updatedAt to {
            AddTextField(
        ...
        DataTypeFormProps.description to {
            AddTextField(
        ...
        DataTypeFormProps.sortIndex to {
            AddTextField(
...
class DataTypeFormDsl(
    val state: MutableState<DataTypeIso>,
    ...
    fun createdAt(
        ...
        order: Int? = null,
        render: (@Composable (MutableState<DataTypeIso>) -> Unit)? = null
    ) {
        when {
            hidden -> {
    ...
    fun insertBefore(targetField: String, vararg newFields: String) {
        if (fieldOrder.isEmpty()) {
            fieldOrder.addAll(DataTypeFormProps.getAllFields())
        }
    ...
    private fun updateFieldOrder(fieldName: String, orderValue: Int) {
        ...
        val allFields = DataTypeFormProps.getAllFields()
...
@Composable
fun rememberDataTypeFormState(current: DataTypeIso? = null): MutableState<DataTypeIso> {
    return remember(current) { mutableStateOf(current ?: DataTypeIso()) }
}