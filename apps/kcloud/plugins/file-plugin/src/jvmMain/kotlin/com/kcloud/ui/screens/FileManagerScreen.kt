...
import com.kcloud.ui.components.EmptyFilesView
import com.kcloud.ui.components.FileGridView
import com.kcloud.ui.components.FileItemViewModel
import com.kcloud.ui.components.FileListView
import com.kcloud.ui.components.FileManagerToolbar
import com.kcloud.ui.components.PathBreadcrumb
import com.kcloud.ui.components.SortBy
import com.kcloud.ui.components.ViewMode
    ...
    Column(
        ...
        selectedEntry?.let { entry ->
            SelectedEntryCard(
                ...
                onShowInFolder = {
                    status = showInFolder(entry.path)
                },
...
private fun buildVisibleEntries(
    ...
    val directoryEntries = directoryChildren.map { (path, childRecords) ->
        FileItemViewModel(
            ...
            lastModified = childRecords.mapNotNull { it.lastSyncTime }.maxOrNull()?.let(::Date),
            extension = ""
        )
    }
...
private fun compareValuesBy(sortBy: SortBy): Comparator<FileItemViewModel> {
    return Comparator { left, right ->
        when (sortBy) {
            ...
            SortBy.DATE -> (right.lastModified?.time ?: 0L).compareTo(left.lastModified?.time ?: 0L)
...
private fun FileWorkspaceRecord.toViewModel(): FileItemViewModel {
    return FileItemViewModel(
        ...
        syncState = syncState,
        lastModified = lastSyncTime?.let(::Date),
...
private fun showInFolder(path: String): String {
    val localFile = File(File(System.getProperty("user.home"), "MoveOff"), path)
    if (!localFile.exists()) {
        return "本地文件不存在：${localFile.absolutePath}"
    }

    return runCatching {
        Desktop.getDesktop().open(localFile.parentFile ?: localFile)
        "已在系统文件管理器中打开：${localFile.parent}"
    }.getOrElse {
        "打开目录失败：${it.message ?: localFile.absolutePath}"
    }
}
