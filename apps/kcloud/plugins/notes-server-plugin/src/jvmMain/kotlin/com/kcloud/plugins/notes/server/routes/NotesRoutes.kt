package com.kcloud.plugins.notes.server.routes
...
fun Routing.installNotesRoutes(
    noteStoreService: NoteStoreService
) {
    route("/api/notes") {
        get("/settings") {
            call.respond(noteStoreService.readSettings())
        }

        put("/settings") {
            val request = call.receive<StorageSettingsUpdateRequest>()
            call.respond(noteStoreService.updateSettings(request))
        }
        ...
        put("/{source}/{id}") {
            ...
            val saved = store.upsertNote(
                NotePayload(
                    id = if (request.id.isBlank()) noteId else request.id,
                    path = request.path,
                    title = request.title,
