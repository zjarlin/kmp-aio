package site.addzero.esp32_host_computer.generated.modbus.tcp
...
import site.addzero.device.driver.modbus.tcp.ModbusTcpConfigRegistry
import site.addzero.device.driver.modbus.tcp.ModbusTcpEndpointConfig
import site.addzero.device.driver.modbus.tcp.ModbusTcpExecutor
 ...
 */
@Serializable
data class DeviceApiTcpGet24PowerLightsRequest(
...
) : GeneratedModbusTcpRequestConfig

/**
 * 读取板子的固件版本、CPU、晶振、Flash 容量和 MAC 地址。
 */
@Serializable
data class DeviceApiTcpGetDeviceInfoRequest(
override val host: String? = null,
override val port: Int? = null,
override val unitId: Int? = null,
override val timeoutMs: Long? = null,
 ...
 */
@Serializable
data class DeviceApiTcpGetFlashConfigRequest(
override val host: String? = null,
override val port: Int? = null,
override val unitId: Int? = null,
override val timeoutMs: Long? = null,
 ...
 * MCU 默认协议桥接
 */
class DeviceApiGeneratedTcpConfigProvider : ModbusTcpConfigProvider {
    override val serviceId: String = "device"

    override fun defaultConfig(): ModbusTcpEndpointConfig =
        ModbusTcpEndpointConfig(serviceId = serviceId, host = "127.0.0.1", port = 502, unitId = 1, timeoutMs = 1000, retries = 2)
}

/**
 * 接管 mcu-console 当前默认的设备读写契约。
 *
 * 该 gateway 由 KSP 自动生成，负责把高阶 Kotlin 接口翻译成 Modbus TCP 调用。
 */
class DeviceApiGeneratedTcpGateway(private val configRegistry: ModbusTcpConfigRegistry, private val executor: ModbusTcpExecutor) : site.addzero.kcloud.plugins.mcuconsole.modbus.device.DeviceApi {
    fun defaultConfig(): ModbusTcpEndpointConfig = configRegistry.require("device")

    private fun resolveConfig(config: ModbusTcpEndpointConfig?): ModbusTcpEndpointConfig =
    ...
    suspend fun get24PowerLights(config: ModbusTcpEndpointConfig? = null): site.addzero.kcloud.plugins.mcuconsole.modbus.device.Device24PowerLightsRegisters {
    ...
    suspend fun getDeviceInfo(config: ModbusTcpEndpointConfig? = null): site.addzero.kcloud.plugins.mcuconsole.modbus.device.DeviceRuntimeInfoRegisters {
        val resolvedConfig = resolveConfig(config)
    ...
    suspend fun getFlashConfig(config: ModbusTcpEndpointConfig? = null): site.addzero.kcloud.plugins.mcuconsole.modbus.device.FlashConfigRegisters {
        val resolvedConfig = resolveConfig(config)
 ...
 */
@Serializable
data class DeviceWriteApiTcpWriteIndicatorLightsRequest(
override val host: String? = null,
override val port: Int? = null,
override val unitId: Int? = null,
override val timeoutMs: Long? = null,
 ...
 */
@Serializable
data class DeviceWriteApiTcpWriteFlashConfigRequest(
override val host: String? = null,
override val port: Int? = null,
override val unitId: Int? = null,
override val timeoutMs: Long? = null,
 ...
 * MCU 默认协议桥接
 */
class DeviceWriteApiGeneratedTcpConfigProvider : ModbusTcpConfigProvider {
 ...
 * 该 gateway 由 KSP 自动生成，负责把高阶 Kotlin 接口翻译成 Modbus TCP 调用。
 */
class DeviceWriteApiGeneratedTcpGateway(private val configRegistry: ModbusTcpConfigRegistry, private val executor: ModbusTcpExecutor) : site.addzero.kcloud.plugins.mcuconsole.modbus.device.DeviceWriteApi {
    fun defaultConfig(): ModbusTcpEndpointConfig = configRegistry.require("device-write")
    ...
    suspend fun writeIndicatorLights(config: ModbusTcpEndpointConfig? = null, faultLightOn: Boolean, runLightOn: Boolean): site.addzero.device.protocol.modbus.model.ModbusCommandResult {
        val resolvedConfig = resolveConfig(config)
    ...
    suspend fun writeFlashConfig(config: ModbusTcpEndpointConfig? = null, magicWord: Int, portConfig: ByteArray, uartParams: ByteArray, slaveAddress: Int, debounceParams: ByteArray, modbusInterval: Int, wdtEnable: Int, firmwareUpgrade: Int, diHardwareFirmware: ByteArray, diStatus: ByteArray, faultStatus: Int, crc: Int): site.addzero.device.protocol.modbus.model.ModbusCommandResult {
        val resolvedConfig = resolveConfig(config)
...
/**
 * Modbus TCP 自动生成的 Koin 模块。
 *
 * 统一收口生成出来的默认配置提供器、配置注册表与网关。
 */
@Module
class GeneratedModbusTcpKoinModule {
    @Single
    fun deviceApiGeneratedTcpConfigProvider(): DeviceApiGeneratedTcpConfigProvider = DeviceApiGeneratedTcpConfigProvider()

    @Single
    fun deviceWriteApiGeneratedTcpConfigProvider(): DeviceWriteApiGeneratedTcpConfigProvider = DeviceWriteApiGeneratedTcpConfigProvider()

    @Single
    fun modbusTcpConfigRegistry(
        deviceApiGeneratedTcpConfigProvider: DeviceApiGeneratedTcpConfigProvider,
        deviceWriteApiGeneratedTcpConfigProvider: DeviceWriteApiGeneratedTcpConfigProvider
    ): ModbusTcpConfigRegistry =
        ModbusTcpConfigRegistry(listOf(deviceApiGeneratedTcpConfigProvider, deviceWriteApiGeneratedTcpConfigProvider))

    @Single
    fun deviceApiGeneratedTcpGateway(
        configRegistry: ModbusTcpConfigRegistry,
        executor: ModbusTcpExecutor,
    ): DeviceApiGeneratedTcpGateway = DeviceApiGeneratedTcpGateway(configRegistry, executor)

    @Single
    fun deviceApi(
        gateway: DeviceApiGeneratedTcpGateway,
    ): site.addzero.kcloud.plugins.mcuconsole.modbus.device.DeviceApi = gateway

    @Single
    fun deviceWriteApiGeneratedTcpGateway(
        configRegistry: ModbusTcpConfigRegistry,
        executor: ModbusTcpExecutor,
    ): DeviceWriteApiGeneratedTcpGateway = DeviceWriteApiGeneratedTcpGateway(configRegistry, executor)

    @Single
    fun deviceWriteApi(
        gateway: DeviceWriteApiGeneratedTcpGateway,
    ): site.addzero.kcloud.plugins.mcuconsole.modbus.device.DeviceWriteApi = gateway

}

fun Route.registerGeneratedModbusTcpRoutes() {
    post("/api/modbus/tcp/device/get24-power-lights") {
        val request = call.receive<DeviceApiTcpGet24PowerLightsRequest>()
        val gateway = KoinPlatform.getKoin().get<DeviceApiGeneratedTcpGateway>()
        val config = request.toEndpointConfig(gateway.defaultConfig())
        call.respond(gateway.get24PowerLights(config = config))
    }
    post("/api/modbus/tcp/device/get-device-info") {
        val request = call.receive<DeviceApiTcpGetDeviceInfoRequest>()
        val gateway = KoinPlatform.getKoin().get<DeviceApiGeneratedTcpGateway>()
        val config = request.toEndpointConfig(gateway.defaultConfig())
        call.respond(gateway.getDeviceInfo(config = config))
    }
    post("/api/modbus/tcp/device/get-flash-config") {
        val request = call.receive<DeviceApiTcpGetFlashConfigRequest>()
        val gateway = KoinPlatform.getKoin().get<DeviceApiGeneratedTcpGateway>()
        val config = request.toEndpointConfig(gateway.defaultConfig())
        call.respond(gateway.getFlashConfig(config = config))
    }
    post("/api/modbus/tcp/device-write/write-indicator-lights") {
        val request = call.receive<DeviceWriteApiTcpWriteIndicatorLightsRequest>()
        val gateway = KoinPlatform.getKoin().get<DeviceWriteApiGeneratedTcpGateway>()
        val config = request.toEndpointConfig(gateway.defaultConfig())
        call.respond(gateway.writeIndicatorLights(config = config, faultLightOn = request.faultLightOn, runLightOn = request.runLightOn))
    }
    post("/api/modbus/tcp/device-write/write-flash-config") {
        val request = call.receive<DeviceWriteApiTcpWriteFlashConfigRequest>()
        val gateway = KoinPlatform.getKoin().get<DeviceWriteApiGeneratedTcpGateway>()
        val config = request.toEndpointConfig(gateway.defaultConfig())
