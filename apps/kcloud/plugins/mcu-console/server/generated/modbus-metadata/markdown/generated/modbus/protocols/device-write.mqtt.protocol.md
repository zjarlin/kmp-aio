# DeviceWriteApi 协议说明（Modbus MQTT）
...
- 固件侧不要修改 `device_write_generated.c`、`modbus_mqtt_dispatch.c` 和 adapter。
...
| `modbus_mqtt_dispatch.h/.c` | 聚合 dispatch，负责按 address 路由到各个 service。 |
...
| --- | --- |
| 连接模式 | 选择 `MQTT`。 |
| Broker | 默认 `tcp://127.0.0.1:1883`。 |
| Client ID | 默认 `modbus-mqtt-client`。 |
| 请求 Topic | 默认 `modbus/request`。 |
| 响应 Topic | 默认 `modbus/response`。 |
| QoS | 默认 `1`。 |
| 超时 | 建议先填 `1000 ms`。 |
...
| Client ID | `modbus-mqtt-client` |
| Request Topic | `modbus/request` |
| Response Topic | `modbus/response` |
| QoS | `1` |
| Timeout Ms | `1000` |
| Retries | `2` |
