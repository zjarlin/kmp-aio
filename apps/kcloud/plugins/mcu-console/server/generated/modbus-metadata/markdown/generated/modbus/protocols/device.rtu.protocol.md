...
- 固件侧不要修改 `device_generated.c`、`modbus_rtu_dispatch.c` 和 adapter。
...
| `modbus_rtu_dispatch.h/.c` | 聚合 dispatch，负责按 address 路由到各个 service。 |
...
| --- | --- |
| 连接模式 | 选择 `RTU`。 |
| 串口 | 填写真实串口或仿真串口，默认示例是 `/dev/ttyUSB0`。 |
| 从站地址 | 默认 `1`；若现场改过 `Unit ID`，这里同步改成现场值。 |
| 波特率 | 默认 `9600`。 |
| 数据位 | 默认 `8`。 |
| 停止位 | 默认 `1`。 |
| 校验 | 默认 `none`。 |
...
| 原始请求帧 | `SEND >>>>>>>>>> 01 01 00 00 00 18 3C 00` |
| 原始响应帧 | `RESPONSE>>>>>>>>>> 01 01 03 00 00 00 3C 4E` |
| 报文说明 | 示例响应里数据区全部按 `0` 演示；实际联调时每一位对应一个线圈状态。 |

### RTU 报文示例

```text
SEND >>>>>>>>>> 01 01 00 00 00 18 3C 00
RESPONSE>>>>>>>>>> 01 01 03 00 00 00 3C 4E
```

- 示例响应里数据区全部按 `0` 演示；实际联调时每一位对应一个线圈状态。
- 请求 payload 示例：`00 00 00 18`
- 响应 payload 示例：`03 00 00 00`

### RTU 请求帧拆解

| 项目 | 从站地址 | 功能码 | payload: 起始地址 | payload: 数量 | CRC16 |
| --- | --- | --- | --- | --- | --- |
| 长度 | `1 byte` | `1 byte` | `2 byte` | `2 byte` | `2 byte` |
| 示例 | `01` | `01` | `00 00` | `00 18` | `3C 00` |
| 归属 | 协议头 | 协议头 | payload | payload | 协议尾 |
| 说明 | 默认 `Unit ID=1`。 | 读取线圈，标准码值 `0x01`。 | 运行时 `address=0` 的大端表示。 | 运行时 `quantity=24` 的大端表示。 | Modbus RTU CRC16 校验码。 |
...
| 原始请求帧 | `SEND >>>>>>>>>> 01 04 00 64 00 1D 71 DC` |
| 原始响应帧 | `RESPONSE>>>>>>>>>> 01 04 3A 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 42 47` |
| 报文说明 | 示例响应里寄存器数据全部按 `0x0000` 演示；实际联调时按下面字段表解码。 |

### RTU 报文示例

```text
SEND >>>>>>>>>> 01 04 00 64 00 1D 71 DC
RESPONSE>>>>>>>>>> 01 04 3A 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 42 47
```

- 示例响应里寄存器数据全部按 `0x0000` 演示；实际联调时按下面字段表解码。
- 请求 payload 示例：`00 64 00 1D`
- 响应 payload 示例：`3A 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00`
...
| --- | --- | --- | --- | --- | --- |
| 长度 | `1 byte` | `1 byte` | `2 byte` | `2 byte` | `2 byte` |
| 示例 | `01` | `04` | `00 64` | `00 1D` | `71 DC` |
| 归属 | 协议头 | 协议头 | payload | payload | 协议尾 |
| 说明 | 默认 `Unit ID=1`。 | 读取输入寄存器，标准码值 `0x04`。 | 运行时 `address=100` 的大端表示。 | 运行时 `quantity=29` 的大端表示。 | Modbus RTU CRC16 校验码。 |
...
| 原始请求帧 | `SEND >>>>>>>>>> 01 03 00 C8 00 21 04 2C` |
| 原始响应帧 | `RESPONSE>>>>>>>>>> 01 03 42 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 7F A6` |
| 报文说明 | 示例响应里寄存器数据全部按 `0x0000` 演示；实际联调时按下面字段表解码。 |

### RTU 报文示例

```text
SEND >>>>>>>>>> 01 03 00 C8 00 21 04 2C
RESPONSE>>>>>>>>>> 01 03 42 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 7F A6
```

- 示例响应里寄存器数据全部按 `0x0000` 演示；实际联调时按下面字段表解码。
- 请求 payload 示例：`00 C8 00 21`
- 响应 payload 示例：`42 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00`
...
| --- | --- | --- | --- | --- | --- |
| 长度 | `1 byte` | `1 byte` | `2 byte` | `2 byte` | `2 byte` |
| 示例 | `01` | `03` | `00 C8` | `00 21` | `04 2C` |
| 归属 | 协议头 | 协议头 | payload | payload | 协议尾 |
| 说明 | 默认 `Unit ID=1`。 | 读取保持寄存器，标准码值 `0x03`。 | 运行时 `address=200` 的大端表示。 | 运行时 `quantity=33` 的大端表示。 | Modbus RTU CRC16 校验码。 |
