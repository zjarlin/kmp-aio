...
static void modbus_rtu_dispatch_set_result(modbus_rtu_dispatch_command_result_t *out_result, bool accepted, const char *summary) {
    if (out_result == NULL) {
        return;
    }
...
bool modbus_rtu_dispatch_write_single_coil(uint16_t address, bool value, modbus_rtu_dispatch_command_result_t *out_result) {
    ...
    switch (address) {
        default:
            modbus_rtu_dispatch_set_result(out_result, false, "unsupported write single coil address");
            return false;
    }
}
    ...
    switch (start_address) {
        case DEVICE_WRITE_WRITE_INDICATOR_LIGHTS_ADDRESS: {
            if (quantity != DEVICE_WRITE_WRITE_INDICATOR_LIGHTS_QUANTITY) {
                modbus_rtu_dispatch_set_result(out_result, false, "write-indicator-lights coil quantity mismatch");
                return false;
            }
            ...
            modbus_rtu_dispatch_set_result(out_result, service_result.accepted, service_result.summary);
            return handled;
        }
        default:
            modbus_rtu_dispatch_set_result(out_result, false, "unsupported write multiple coils address");
            return false;
    }
}
...
bool modbus_rtu_dispatch_write_single_register(uint16_t address, uint16_t value, modbus_rtu_dispatch_command_result_t *out_result) {
    ...
    switch (address) {
        default:
            modbus_rtu_dispatch_set_result(out_result, false, "unsupported write single register address");
            return false;
    }
}
    ...
    switch (start_address) {
        case DEVICE_WRITE_WRITE_FLASH_CONFIG_ADDRESS: {
            if (quantity != DEVICE_WRITE_WRITE_FLASH_CONFIG_QUANTITY) {
                modbus_rtu_dispatch_set_result(out_result, false, "write-flash-config register quantity mismatch");
                return false;
            }
            device_write_command_result_t service_result = {0};
        ...
        default:
            modbus_rtu_dispatch_set_result(out_result, false, "unsupported write registers address");
            return false;
    }
}
