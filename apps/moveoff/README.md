...
MoveOff 的回答是：**只存一份，但处处可用。** 用同步代替复制，用统一代替分散。你的配置、你的脚本、你的工作流，只维护一个源头，然后让它们像单例一样，在你所有设备、所有项目中自动就位。
...
MoveOff 最终形态不是 Dropbox，而是一个**声明式配置管理器**——你声明"我需要这些工具、这些配置"，它负责让它们在你所有设备上以正确的形式存在。
