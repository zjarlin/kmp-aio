...
class EncryptedStorageClient(
    private val delegate: com.moveoff.sync.api.StorageClient,
    private val encryptionManager: EncryptionManager,
    private val passwordProvider: () -> String
) : com.moveoff.sync.api.StorageClient {

    override suspend fun testConnection(): Boolean {
        return delegate.testConnection()
    }

    override suspend fun listObjects(prefix: String?): List<com.moveoff.sync.api.RemoteObject> {
    ...
    override suspend fun uploadObject(
    ...
    ): com.moveoff.sync.api.UploadResult {
        val originalFile = File(localPath)
        if (!originalFile.exists()) {
            return com.moveoff.sync.api.UploadResult(success = false, error = "文件不存在: $localPath")
        }
        ...
        return try {
            ...
            if (encryptResult.isFailure) {
                return com.moveoff.sync.api.UploadResult(
                    success = false,
    ...
    override suspend fun downloadObject(
        ...
        localPath: String,
        progress: (Long, Long) -> Unit
    ): com.moveoff.sync.api.DownloadResult {
        val encryptedRemotePath = "$remotePath.enc"
        val tempEncryptedFile = File.createTempFile("moveoff_enc_", ".tmp")
        ...
        return try {
            ...
            if (decryptResult.isFailure) {
                return com.moveoff.sync.api.DownloadResult(
                    success = false,
                    error = "解密失败: ${decryptResult.exceptionOrNull()?.message}"
                )
            }

            com.moveoff.sync.api.DownloadResult(success = true, bytesDownloaded = downloadResult.bytesDownloaded)
        } finally {
            tempEncryptedFile.delete()
        }
    }
    ...
    override suspend fun getObjectMetadata(remotePath: String): com.moveoff.sync.api.RemoteObject? {
        // 优先获取加密文件的元数据
