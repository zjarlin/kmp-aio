...
internal class DampedDragAnimation(
    ...
    fun updateValue(value: Float) {
        val nextValue = value.coerceIn(valueRange)
        animationScope.launch {
            launch {
                valueAnimation.animateTo(nextValue, valueAnimationSpec) {
                    updateVelocity()
                }
            }
        }
    ...
    fun animateToValue(value: Float) {
        animationScope.launch {
            mutatorMutex.mutate {
                press()
                val nextValue = value.coerceIn(valueRange)
                launch { valueAnimation.animateTo(nextValue, valueAnimationSpec) }
                if (velocity != 0f) {
    ...
    private fun updateVelocity() {
        ...
        val nextVelocity = velocityTracker.calculateVelocity().x / (valueRange.endInclusive - valueRange.start)
        animationScope.launch {
            velocityAnimation.animateTo(nextVelocity, velocityAnimationSpec)
        }
    }
}
