...
- `site.addzero:config-center`
  只负责通用契约、`ConfigCenterEnv`、JDBC 存储、Koin 运行时装配。
...
- 宿主直接提供一个自己应用的 `ConfigCenterEnv` bean
- 其他地方直接注入 `ConfigCenterEnv`
...
虽然 `ConfigCenterEnv` 提供了 `boolean`、`list`、`map` 这类读取辅助，但这只是消费侧解析能力，不代表存储层存在独立类型系统。
...
- `site.addzero.configcenter.ConfigCenterEnv`
